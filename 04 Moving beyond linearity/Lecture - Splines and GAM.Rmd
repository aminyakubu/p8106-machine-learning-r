...
From the distribution we see that `pgg45` looks nonlinear.
...
fit2 <- lm(lpsa~poly(pgg45,2), data = Prostate) # you can use raw= TRUE in the poly() function to use squared or cubic values. The results will be similar. 
...
We see from anova the model 2 is better than model 1. But the subsequent models are not better than model 2. 

## Step function
The function `cut()` can be used to create step function basis. The argument `breaks` can be used to specify the cutpoints.
```{r}
fit.sf <- lm(lpsa~cut(pgg45, 4), data = Prostate) # We use the cut function to create a step function. We used breaks = 4 to create 4 break points. We can use a vector like c(25, 50) to represent where we want the cuts to be. 
```
...
The function `bs()` can be used to generate the spline basis matrix. You can specify the degree of freedom via `df`; you may also use the `knots` and `degree` arguments. Defaut is degree = 3 (cubic). It's simpler to use degrees of freedom than knots. 
...
## Natural cubic splines - when the end of the CI of cubic splines are too wide. Natural cub. splines put constrains at the ends
...
```{r}
fit.ss <- smooth.spline(Prostate$pgg45, Prostate$lpsa) # This will automatically select the optimal degree of freedom 
fit.ss$df
